...
// Routes
app.get('/', (req, res) => {
    res.sendFile(new URL('../public_html/crackerbot.html', import.meta.url).pathname);
});
...
async function generateProjectWithClaude(projectName, projectType, features) {
    const projectStructures = {
        "web-app": ["crackerbot.html", "app.js", "styles.css"],
        "game": ["crackerbot.html", "game.js", "style.css"],
    ...
    socket.on('ai_generate_project', async (data) => {
        try {
            ...
            if (aiHandlers && aiHandlers.generateProject) {
                const project = { files: await generateProjectWithClaude(data.name, data.type, data.features || data.requirements || "") };
                ...
                console.log('Live editor triggered for:', data.name);
            }
        } catch (error) {
